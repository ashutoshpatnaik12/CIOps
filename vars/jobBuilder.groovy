...
""" 
        ...
        // Use the branch selected in the Job Builder job.
        // Default to master if no branch is supplied.
        String jobBuilderBranch = params.get('branch', 'master');

        // Git Parameter returns values like origin/master-rep-niua.
        // Remove origin/ before passing the branch to Jenkins git checkout.
        jobBuilderBranch = jobBuilderBranch.replaceFirst(/^origin\//, '');

        echo "Job Builder Branch : ${jobBuilderBranch}";
        ...
        for (int i = 0; i < gitUrls.size(); i++) { 
            ...
            dir(dirName) {
                 ...
                 // Checkout the selected UPYOG branch.
                 git branch: jobBuilderBranch,
                     url: gitUrls[i],
                     credentialsId: 'git_read'

                 // build-config.yml will now be read from the selected branch.
...
"""); 
        ...
        stage('Building jobs') {
           ...
           echo "Creating Jenkins jobs from branch: ${jobBuilderBranch}";
